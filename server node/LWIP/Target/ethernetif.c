...
/* Stack size in bytes (CMSIS-RTOS2). 350 was far too small — corrupts stack and breaks ARP/ICMP randomly. */
...
/* USER CODE END low_level_init Variables Initialization for User BSP */
  ...
  /* Counting semaphore: each RX IRQ does GiveFromISR; a binary semaphore drops extra gives if two
   * frames arrive before EthIf runs — causes lost wakeups and flaky ping/ARP. */
  RxPktSemaphore = osSemaphoreNew((uint32_t)ETH_RX_BUFFER_CNT, 0U, NULL);
...
/* USER CODE END OS_THREAD_NEW_CMSIS_RTOS_V2 */
  ...
  /* After HAL_ETH_Init the MAC/DMA are still off; Start_IT enables RX/TX and RX IRQs (required for ARP, ping, HTTP). */
...
/* USER CODE BEGIN ETH link Thread core code for User BSP */
    /* Only trust PHY reads that succeed. On timeout, regvalue stays 0 and would wrongly call netif_set_link_down. */
    if (HAL_ETH_ReadPHYRegister(&heth, 0, 1, &regvalue) == HAL_OK)
    {
      uint8_t link_now = ((regvalue & (1U << 2)) != 0U) ? 1U : 0U;
      if (link_now != link_up_prev)
      {
        if (link_now)
        {
          netif_set_link_up(netif);
